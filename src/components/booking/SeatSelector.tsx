...
const SeatSelector: FC<Props> = ({
  ...
  const [pendingWheelchairSeat, setPendingWheelchairSeat] =
    useState<SeatBooking | null>(null);
  ...
  function groupSeatsByRow(seats: Seat[]): Seat[][] {
    ...
    return Object.values(rows)
      .map((row) => row.sort((a, b) => a.seat - b.seat))
  ...
  return (
    <div className='m-auto max-w-[720px] p-4 mb-8 md:p-8 pb-6 space-y-6 bg-gray-900 border-4 border-yellow-400 shadow-[inset_0_0_10px_#facc15,0_0_20px_#facc15]'>
      <div className='flex justify-center mb-6'>
        <div className='w-full h-6 bg-gray-700 rounded-sm shadow-sm text-gray-400 font-bold mb-6 text-center'>
          Bioduk
        </div>
      </div>
      <div
        ...
        role='region'
        aria-label='Välj platser i salongen'
      >
        ...
        <div className='flex flex-col items-center space-y-2 min-w-[360px] p-4'>
          {salong.map((row, rowIndex) => (
            <div
              key={rowIndex}
              className='flex justify-center gap-2 sm:gap-3 min-w-[360px] '
            >
              {row.map((seat, seatIndex) => {
                ...
                return (
                  <button
                    ...
                    className={`w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded text-white ${
                      isBooked
                        ? 'bg-red-500 cursor-not-allowed'
                        : isSelected
                          ? 'bg-green-800 border-3 border-white cursor-pointer'
                          : seat.isWheelchair
      ...
      {bookingSuccess && (
        <p className='text-green-600 mt-2' aria-live='polite'>
          Tack! Platserna är bokade!
        </p>
      )}
      <div
        className='mt-6 space-y-2 text-center'
        role='region'
        aria-label='Stolguide'
      >
      ...
      <div className='flex justify-center mt-6'>
        <button
          ...
          className={`mt-6 px-6 py-3 w-full sm:w-auto rounded text-black font-semibold transition focus:outline focus:outline-white ${
            selectedSeats.length !== maxSeats || isBooking
