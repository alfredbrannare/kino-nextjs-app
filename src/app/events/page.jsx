    ...
    <div className="max-w-5xl mx-auto px-6 py-12 space-y-12">
      ...
      <div className="space-y-10">
        {events.map((event, index) => (
          <div key={index} className="p-6 border border-[#CDCDCD] rounded-xl shadow-sm hover:shadow-lg hover:scale-[1.01] transition transform duration-200">
                        <img
              src={event.image}
              alt={event.title}
              className="w-full h-60 object-cover rounded-lg"
            />
            <h2 className="text-2xl font-semibold text-[#CDCDCD]">{event.title}</h2>
