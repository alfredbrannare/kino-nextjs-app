    ...
    <>
      ...
      {movies.map(movie => (
        <div key={movie._id} className="block mx-auto p-4 mb-3 bg-base-300 flex justify-between max-w-200 ">
          ...
          <div>
            <Link className="btn mr-1" href={`/movies/new/` + movie._id}>Details</Link>
            {movie.inCinemas ? <span className="btn btn-success mr-1" onClick={() => updateMovie(movie._id, false)}>Currently showing</span> : <span className="btn btn-warning mr-1" onClick={() => updateMovie(movie._id, true)}>Currently not showing</span>}
